...
current_algorithms = [f for f in os.listdir(algorithms_directory) if f.endswith(".py")]
...
for el in new_algorithms:
    spec = importlib.util.spec_from_file_location(el[:-3], algorithms_directory+el)
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)

    classes = [cls for cls_name, cls in inspect.getmembers(module, inspect.isclass)]
    new_algorithm = classes[0]()
    ...
    encrypted_file_path = "algorithms_history/"+el[:-3]+".bin"
